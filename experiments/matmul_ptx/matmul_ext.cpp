...
extern "C" int launch_matmul_ptx_mma_cpasync_k32(
    ...
    int64_t k,
    cudaStream_t stream);
extern "C" int launch_matmul_ptx_mma_cpasync_128(
    const at::Half* a,
    const at::Half* b,
    float* c,
    int64_t m,
    ...
    int64_t k,
    cudaStream_t stream);
extern "C" int launch_matmul_ptx_mma_cpasync_128_k32(
    const at::Half* a,
    const at::Half* b,
    float* c,
    int64_t m,
...
torch::Tensor matmul_mma_cpasync_128(torch::Tensor a, torch::Tensor b) {
    return run_launcher(a, b, launch_matmul_ptx_mma_cpasync_128);
}

torch::Tensor matmul_mma_cpasync_128_k32(torch::Tensor a, torch::Tensor b) {
    return run_launcher(a, b, launch_matmul_ptx_mma_cpasync_128_k32);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("matmul_mma_cpasync_128", &matmul_mma_cpasync_128, "PTX MMA cp.async matmul with 128x128 CTA tile (CUDA)");
    m.def("matmul_mma_cpasync_128_k32", &matmul_mma_cpasync_128_k32, "PTX MMA cp.async matmul with 128x128 CTA tile and blockK=32 (CUDA)");
